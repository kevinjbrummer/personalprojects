...
#simulation.randomize(simulation.grid_model, simulation.height,
                            # simulation.width)
def setup():
    ...
    grid_view = Canvas(root, width=simulation.width*cell_size,
                       height=simulation.height*cell_size,
                       borderwidth=0,
    ...
    option = OptionMenu(root, choice, 'Choose a Pattern', 'glider', 'glider gun',
                    'random')
...
def update():
    ...
    simulation.next_gen()
    for i in range(0, simulation.height):
        for j in range(0, simulation.width):
            if (simulation.grid_model[i][j]) == 1:
                draw_cell(i, j, 'black')
    if (is_running):
...
def clear_handler(event):
    ...
    is_running = False
    grid_view.delete(ALL)
    for i in range(simulation.height):
        for j in range(simulation.width):
            simulation.grid_model[i][j] = 0
    start_button.configure(text='Start')
    update()
...
def grid_handler(event):
    ...
    if simulation.grid_model[x][y] == 1:
        simulation.grid_model[x][y] = 0
        draw_cell(x, y, 'white')
    else:
        simulation.grid_model[x][y] = 1
        draw_cell(x, y, 'black')
